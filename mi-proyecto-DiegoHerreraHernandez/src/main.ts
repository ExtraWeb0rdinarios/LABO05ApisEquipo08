...
const fetchCommentsByPost = async (postId: number): Promise<void> => {
  if(IS_DEBUG_MODE){
    console.log(`%c [LAB 3 Buscando los comentarios del post con id ${postId} ...`, "color: cyan; font-weight: bold;");
  }
  ...
  // Usa 'fetch' con backticks para unir API_URL + /posts/ + postId + /comments.
    ...
    // Dentro, imprime solo el 'email' de cada comentario para verificar el tipado.
      console.log(`   - Emails de Comentarios Recuperados:`);
      data.forEach((comentario: Comment,index: number) =>{
        console.log(` ${index+1}. ${comentario.email}`)
      });
